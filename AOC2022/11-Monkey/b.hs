...
calcCommonMod :: [[String]] -> Int
calcCommonMod arr = foldr (*) 1 $ map (\strs -> read $last $ words $ strs !! 3) arr 
...
monkeyInspect (Monkey (item:items') opf testf (iftrue,iffalse) toThrow counter)
    ...
    where afterop    = opf item
          test       = testf afterop :: Bool
          target     = if test then iftrue else iffalse
          toThrow'   = toThrow ++ [(target, afterop)]
...
addItemToMonkey val (Monkey items opf testf (iftrue,iffalse) toThrow counter)
    = (Monkey (items ++ [mod val 9699690]) opf testf (iftrue,iffalse) toThrow counter)
...
main = do
    ...
    let lin = lines cont
        ...
        commonMod = calcCommonMod grouped
        afterroundX00 = iter 10000 (\m -> doRound counter m) monkeys
        counters     = map getMonkeyCounter afterroundX00
        sortedcounters = reverse $ sort counters
    print commonMod
    mapM_ print $ map getMonkeyItems monkeys
    print "----- X00 ----"
    mapM_ print $ map getMonkeyItems afterroundX00
    print "-----"
