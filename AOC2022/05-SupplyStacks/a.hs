...
moveOnce :: Int -> Int -> Board -> Board
moveOnce frid toid board = updateBoard (frid,frstr') $ updateBoard (toid, tostr') board
    where (_, frstr) = board !! (frid - 1)
          (_, tostr) = board !! (toid - 1)
          picked  = last $ frstr :: Char
          frstr'  = init $ frstr :: String
          tostr'  = tostr ++ [picked]

moveN :: Int -> Int -> Int -> Board -> Board
moveN _     _ 0 = id
moveN from to n = moveN from to (n-1) . moveOnce from to
