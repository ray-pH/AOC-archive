...
fn get_antinodes_inside_map(antennas: &Vec<Pos>, map: &Map) -> HashSet<Pos> {
...
fn get_continuous_antinodes_inside_map(antennas: &Vec<Pos>, map: &Map) -> HashSet<Pos> {
    let mut result = HashSet::new();
    for i in 0..antennas.len() {
        for j in i+1..antennas.len() {
...
fn parse_input(input: &String) -> Map {
    ...
    for (row, line) in input.lines().enumerate() {
        for (col, c) in line.chars().enumerate() {
            if c != '.' {
                if !antennas_freq_map.contains_key(&c) {
                    antennas_freq_map.insert(c, Vec::new());
                }
