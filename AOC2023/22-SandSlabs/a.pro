...
keep_dropping(Names, PosAssoc-BlockAssoc, NewPosAssoc-NewBlockAssoc) :-
    member(Name, Names),
    ...
    keep_dropping(Names, NextPosAssoc-NextBlockAssoc, NewPosAssoc-NewBlockAssoc).
    % otherwise, stop
keep_dropping(_, PosAssoc-BlockAssoc, PosAssoc-BlockAssoc).
...
% generate_empty_assoc(Names, Assoc) :-
%     empty_assoc(EmptyAssoc),
%     foldl([Name,AssocIn,AssocOut]>>(put_assoc(Name, AssocIn, [], AssocOut)), Names, EmptyAssoc, Assoc).
...
main :-
    ...
    keep_dropping(Names, PosAssoc-BlockAssoc, FinalPosAssoc-FinalBlockAssoc),
    exclude(cannot_be_disintregated(Names, FinalPosAssoc-FinalBlockAssoc), Names, DisintregableNames),
    write(DisintregableNames), nl,
    length(DisintregableNames, NDisintregable),
    write(NDisintregable),
    nl.
