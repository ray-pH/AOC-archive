...
t_head = tuple[t_pos, str, int, int, list[t_pos]]
t_map = list[list[int]]
...
# def is_valid_pos_dir(pos : t_pos, dir: str, map : t_map) -> bool:
#     row_count = len(map)
#     col_count = len(map[0])
#     if pos[0] < 0: return False
#     if pos[1] < 0: return False
#     if pos[0] >= row_count: return False
#     if pos[1] >= col_count: return False
#     if dir == 'v' and pos[1] >= col_count-4: return False
#     if dir == '^' and pos[1] >= col_count-4: return False
#     return True

def is_valid_head(head : t_head, map : t_map) -> bool:
    pos, dir, count, dist, _ = head
    row_count = len(map)
    col_count = len(map[0])
    if pos[0] < 0: return False
    if pos[1] < 0: return False
...
def update_distmap_nextheads(head : t_head, newcount : int, d : str, 
                             map : t_map, distmap : t_map, nextheads : list[t_head]):
    pos, _, _, dist, hist = head
    ...
    if newdist >= prev_dist + 9*4: return
    # newdist < prev_dist
    if newdist < prev_dist:
        distmap[newpos[0]][newpos[1]] = newdist
    newhead = newpos, d, newcount, newdist, hist + [pos]
    nextheads.append(newhead)

# def next_heads0(head : t_head, map : t_map, distmap : t_map) -> set[t_head]:
#     _, dir, count, _ = head
...
#         update_distmap_nextheads(head, newcount, d, map, distmap, nextheads)
#     return nextheads

def next_heads(head : t_head, map : t_map, distmap : t_map) -> list[t_head]:
    _, dir, count, _, _ = head
    nextheads = []
    if count < 10:
...
def prune_heads(heads : list[t_head]) -> list[t_head]:
    ...
    for i in range(1,len(heads)):
        ppos, pdir, pcount, pdist, _ = pruned[-1]
        pos, dir, count, dist, _ = heads[i]
