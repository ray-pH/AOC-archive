...
calc_energized(StartPos-StartDir, Layout, EnergizedCount) :-
    step([StartPos-StartDir], Layout, BeamHistory),!,
...
main :-
    ...
    findall((R,-1)-'>', between(0, Nm1, R), LeftBeams),
    findall((R,N)-'<', between(0, Nm1, R), RightBeams),
    findall((-1,C)-'v', between(0, Nm1, C), TopBeams),
    findall((N,C)-'^', between(0, Nm1, C), BottomBeams),
    append([LeftBeams, RightBeams, TopBeams, BottomBeams], StartingBeams),!,
    map3_debug(calc_energized, StartingBeams, Layout, EnergizedCounts),
