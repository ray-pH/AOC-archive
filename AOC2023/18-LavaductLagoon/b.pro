...
parse_line(Line, Parsed) :-
    split_string(Line, " ", "", [_, _, HexInParen]),
    string_chars(HexInParen, HexChars),
    append([['(','#'], HexCodeChars, [')']], HexChars),
    init(HexCodeChars, NumChars),
    string_chars(NumStr, NumChars),
    string_concat("0x", NumStr, HexNumStr),
    number_string(Num, HexNumStr),
    reverse(HexCodeChars, [DirCode|_]),
    dircode_dir(DirCode, Dir),
    Parsed = Dir-Num-"".
