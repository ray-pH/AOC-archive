...
deploy :: [Link] -> Bool -> [Node] -> Node -> Int
deploy cave smalltwice history pos 
    | pos == "end" = 1
    | pos == "start" && 0 < length history = 0
      -- visited small cave twice (once)
    | not (isStrCapital pos) && elem pos history && not smalltwice = sum $ map (deploy cave True (pos:history)) others
    | not (isStrCapital pos) && elem pos history && smalltwice = 0 -- visited small cave twice (another)
    | otherwise = sum $ map (deploy cave smalltwice (pos:history)) others
...
main = do
    ...
    print $ deploy links False [] "start"
